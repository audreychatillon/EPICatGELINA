...
void run(int run_number, int P, int HV) {
  ...
  EpicPreTreat raw(ch);
    ...
    TFile *fcut = new TFile(Form("v4b_tcutg_discri_%imbar_%iV/fsave_tcutg_discri.root",P,HV),"read");
    for(int a = 0 ; a < nA ; a++){
        for(int c = 0 ; c < ncuts ; c++){
          ...
          string hisname = Form("V4b_SIGNAL_%imbar_%iV_A%02d_cut%02d",P,HV,a+1, c+1);
    ...
    if (q3 > 0) {
      ...
      for(int c = 1 ; c <= ncuts ; c++){
        ...
        if(cuts[cutname]->IsInside(q1,q2/q3)){ 
            string hisname = Form("V4b_SIGNAL_%imbar_%iV_A%02d_cut%02d",P,HV,anode, c);
  ...
  // ===========================================================================================
  // === DRAW
  TFile *fsave = new TFile(Form("v4b_tcutg_discri_%imbar_%iV/fsave_histos.root",P,HV),"recreate");
  ...
  for (int a = 0; a < nA; a++) {
    can1->cd(a + 1);    gPad->SetLogz();  gPad->SetLogx();
    ...
    can6[a]->Divide(3,2);
    for(int c = 1 ; c <= ncuts ; c++){
        string cutname = Form("A%02d_cut%02d",a+1,c);
        can1->cd(a+1); cuts[cutname]->Draw("same");
        string hisname = Form("V4b_SIGNAL_%imbar_%iV_A%02d_cut%02d",P,HV,a+1, c);
